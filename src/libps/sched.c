...
// GPU clock:   33.8685MHz * (11 / 7) = 53.2219MHz
//
// Frame Rates
// PAL:  53.2219MHz / 314 / 3406 = ca. 49.76 Hz (~50Hz)
// NTSC: 53.2219MHz / 263 / 3413 = ca. 59.29 Hz (~60Hz)
//
...
// Adjusts the master clock of the scheduler to `clock_rate` (in Hz) and
// adjusts the other clocks as well.
                                      ...
                                      const unsigned int clock_rate)
{
    ...
    sched->cpu_clock    = sched->master_clock / 2;
    sched->spu_clock    = sched->master_clock / 1536;
...
void libps_scheduler_step(struct libps_scheduler* sched)
{
    ...
    {
        if (sched->tasks[ev].active)
        {
            if (sched->tasks[ev].cycles <= 0)
            {
                sched->tasks[ev].dispatch_func(sched->tasks[ev].device);

                switch (sched->tasks[ev].sync_mode)
                {
                    case LIBPS_SCHEDULER_SYNC_CPU:
                        sched->tasks[ev].cycles = sched->cpu_clock / sched->frame_rate;
                        break;

                    case LIBPS_SCHEDULER_SYNC_GPU:
                        sched->tasks[ev].cycles = sched->gpu_clock / sched->frame_rate;
                        break;
                              ...
                              void* device)
{
    ...
    sched->tasks[ev].active        = true;
    sched->tasks[ev].sync_mode     = sync_mode;
    sched->tasks[ev].dispatch_func = cb;
    sched->tasks[ev].device        = device;
